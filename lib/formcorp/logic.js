  ...
  var init = function (fc) {
    ...
    var getComponent = function (id, belongsTo) {
      if (id.indexOf(formcorp.logic._CONST.GROUPLET_FIELD_SEPARATOR) >= 0) {
        ...
        if (_.isArray(match) && match.length === 4) {
          id = match[3];
          belongsTo = match[1];
        }
      }
    ...
    var isRepeatableGroupletValid = function (grouplet) {
      console.log("CHECK REPEATABLE GROUPLET VALID\n================");
      console.log(grouplet);
    ...
    var isGroupletValid = function (grouplet) {
      ...
      if (repeatable) {
        return isRepeatableGroupletValid(grouplet);
      }
