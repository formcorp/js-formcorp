    ...
    session: function (fieldId) {
      ...
      this.events.subscribe(this.events.const.RENDER_PENDING, function (obj) {
        ...
        if (typeof _self.config.allowSkipping === 'boolean' && _self.config.allowSkipping) {
          skip = template(
            ...
            '</div>'
          , {
            skipText: _self.lang('skipText', 'Can\'t pass verification? <a href="#" class="verification-skip fc-link">Click here</a> to skip.')
          });
        }
      ...
      this.events.subscribe(this.events.const.SOURCE_CHANGED, function () {
        if (!_self.canStillVerify()) {
          ...
          _self.events.publish(_self.events.const.FATAL_ERROR, {
            msg: _self.lang('notEnoughInformation', 'We\'re sorry, but we could not gather enough information to verify your identity.')
          });
        }
      });
      ...
      this.events.subscribe(this.events.const.SOURCE_SUBMIT, function (source) {
        ...
        if (errors) {
          ...
          _self.events.publish(_self.events.const.VERIFICATION_ERROR, {
            error: _self.lang('requiredFieldsError', 'Please ensure all required form fields are correctly filled out prior to verification.')
          });
        } else {
        ...
        fc.api('greenid/gateway-v2/verify?source=' + source.source, data, 'POST', function (result) {
            ...
            if (result.message.indexOf('locked out of') >= 0) {
              _self.updateRemotely(function (data) {
                ...
                _self.events.publish(_self.events.const.RENDER_PENDING, {
                  error: _self.lang('lockedOutTooManyAttempts', 'You\'ve been locked out of the selected data source due to too many failed attempts. Please select a different method.')
                });
              });
          ...
          if (result.success === true) {
            ...
            if (passedSource(status)) {
              ...
              submitGroup.removeClass('verifying');
              _self.events.publish(_self.events.const.VERIFICATION_ERROR, {
                error: _self.lang('verificationFailed', 'We\'re sorry, but electronic identification failed, please try again')
              });
            }
      ...
      this.events.subscribe(this.events.const.FETCH_ERROR, function () {
        ...
        _self.events.publish(_self.events.const.FATAL_ERROR, {
          msg: _self.lang('fetchError', 'An unknown error occurred initialising electronic verification. You may be asked additional questions later on to verify your identity.')
        });
      });
      ...
      this.canStillVerify = function () {
        ...
        var primarySources = _.keyBy(_.filter(_self.options.sources.active, function (o) {
          var status = _self.getSourceStatus(o);
          return o.primary === true && (typeof o.source !== 'string' || _self.userHasSource(o.source)) && status === 'available';
        }), 'key');


        // Remaining sources remotely
        var remainingRemoteSources = _.keyBy(_.filter(_self.getUserVerificationSources(), function (o) {
          return typeof o.passed === 'boolean' && !o.passed && typeof o.state === 'string' && ['EMPTY', 'IN_PROGRESS'].indexOf(o.state) >= 0;
        }), 'name');
