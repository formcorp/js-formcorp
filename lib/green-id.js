...
var fcGreenID = (function ($) {
    ...
    var configKeys = {
         ...
         * @param values
         */
        initialiseGreenIDVerification = function (fieldId, values, callback) {
        ...
        updateIsVerified = function (fieldId) {
            ...
            if (fieldContainer.length === 0) {
                console.log('updateIsVerified field container not found.');
                return;
            }
            
            // Update the DOM with the field verification status
            if (fcGreenID.isVerified(fieldId)) {
                // Verification has succeeded
                fieldContainer.removeClass(notVerifiedClass).removeClass(verificationErrorClass).addClass(verifiedClass);
            } else if (fcGreenID.hasFailed(fieldId)) {
                // Verification has failed
                fieldContainer.removeClass(notVerifiedClass).removeClass(verifiedClass).addClass(verificationErrorClass);
            } else {
                // Verification has not completed
                fieldContainer.removeClass(verifiedClass).removeClass(verificationErrorClass).addClass(notVerifiedClass);
            }
        },
        ...
        verifyPassport = function (fieldId) {
            ...
            fc.api('greenid/gateway/verify-passport', postData, 'POST', function (data) {
                ...
                if (!passedValidation(fieldId, 'passport')) {
                    showError(childContainer, '<strong>Oops!</strong> We were unable to verify your oassoirt. Please double-check your details and try re-submitting.');
                    return;
                }
        ...
        verifyVisa = function (fieldId) {
            ...
            for (key in requiredFields) {
                if (requiredFields.hasOwnProperty(key)) {
                    console.log('.' + requiredFields[key] + ' input');
                    input = childContainer.find('.' + requiredFields[key] + ' input');
    ...
    return {
         ...
         */
        init: function () {
            console.log('greenID init called');
            this.values = {};
            this.initialised = {};
        ...
        getPercentage: function (fieldId) {
            ...
            if (typeof value.result.outcome === 'string' && ['VERIFIED', 'PENDING'].indexOf(value.result.outcome) > -1) {
                return 100;
            }
        ...
        hasFailed: function (fieldId) {
            return !this.isVerified(fieldId) && this.getPercentage(fieldId) >= 80;
        },
        ...
        passesValidation: function (fieldId) {
            console.log(fieldId);
            console.log(this.isVerified(fieldId));
            console.log(this.hasFailed(fieldId));
            return this.isVerified(fieldId) || this.hasFailed(fieldId);
        },
