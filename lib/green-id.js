...
var fcGreenID = (function($) {
  ...
  var getConfigValues = function(field, fieldId) {
    ...
    var lookupValue = function(id) {
      if (fieldId.indexOf(fc.constants.prefixSeparator) >= 0) {
        ...
        var value = source.split('.').reduce(function(o, x) {
  ...
  return {
    ...
    session: function(fieldId) {
      ...
      this.events = (function() {
        ...
        return {
          const: {
            ...
            FETCH_ERROR: 'greenid/fetch/error',
            ...
            SOURCE_CHANGED: 'greenid/sources/changed',
            ...
            VERIFICATION_SKIPPED: 'greenid/verification/skipped',
      ...
      // Fatal error during initialisation. Should allow user to pass on fatal error
      ...
      this.events.subscribe(this.events.const.FETCH_RESULT_INITIALISED, function(obj) {
        ...
        if (typeof obj === 'object' && typeof obj.result === 'object') {
          ...
          fc.setVirtualValue(_self.fieldId, obj.result, fc.fields, false, false);
        }
      ...
      this.events.subscribe(this.events.const.EXTRA_FAILED, function(obj) {
        var html = template(
            ...
            '</div>',
          {
            ...
            text: _self.lang('extraFailContinue', 'Continue'),
          },
        );
      ...
      this.events.subscribe(this.events.const.READY_FOR_RENDER, function(obj) {
        if (_self.isSkipped()) {
          // User has previously skipped verification
          _self.events.publish(_self.events.const.VERIFICATION_SKIPPED);
          return;
        }
        ...
        switch (status) {
          ...
          case GREENID.STATE.REJECTED:
            _self.events.publish(_self.events.const.FATAL_ERROR, {
              msg: _self.lang('couldNotVerify', "We're sorry, but we could not verify your identity."),
            });
            break;
      ...
      this.events.subscribe(this.events.const.RENDER_PENDING, function(obj) {
        ...
        _.each(sources, function(source) {
          ...
          sourceHtml += template(
            '<li class="fc-greenid-source" data-status="<%this.status%>" data-source="' +
              source.key +
              '">' +
        ...
        // If can skip, add a skip button
        var skip = '';
        if (typeof _self.config.allowSkipping === 'boolean' && _self.config.allowSkipping) {
          skip = template(
            '<div class="skip-verification">' + ' <div>' + '   <%this.skipText%>' + ' </div>' + '</div>',
            {
              skipText: _self.lang(
                'skipText',
                'Can\'t pass verification? <a href="#" class="verification-skip fc-link">Click here</a> to skip.',
              ),
            },
          );
        }
        ...
        // The sources have potentially changed
        _self.events.publish(_self.events.const.SOURCE_CHANGED);
      });
      ...
      this.events.subscribe(this.events.const.RENDER_VERIFIED, function(obj) {
        ...
        _self.events.publish(_self.events.const.VERIFICATION_SUCCESS, {
          msg: _self.lang('verifyMessage', 'Your identify has successfully been verified.'),
        });
      });

      // Sources have changed
      this.events.subscribe(this.events.const.SOURCE_CHANGED, function() {
        if (!_self.canStillVerify()) {
          // Can't verify, output an error
          _self.events.publish(_self.events.const.FATAL_ERROR, {
            msg: _self.lang(
              'notEnoughInformation',
              "We're sorry, but we could not gather enough information to verify your identity.",
            ),
          });
        }
      });
      ...
      this.events.subscribe(this.events.const.FATAL_ERROR, function(obj) {
        var html = template('<div class="fc-green-id fatal-error">' + ' <div><%this.msg%></div>' + '</div>', {
          msg: obj.msg,
        });
      ...
      this.events.subscribe(this.events.const.DISPLAY_SOURCE, function(obj) {
        _self.render(_self.loading(_self.lang('retrievingSourceDetails', 'Retrieving source details')));
        ...
        fc.api('greenid/gateway-v2/get-fields', data, 'POST', function(obj) {
          ...
          var htmlTemplate = template(
            '<div class="fc-green-id-summary"><%this.summary%></div>' +
              ...
              '   </div>' +
              '   <div class="source-submit">' +
              '     <div class="submit" data-source="' +
              source.key +
              '"><%this.lang.submit%></div>' +
              '     <div class="verifying">' +
            ...
            {
              lang: {
                submit: _self.lang('verify', 'Verify'),
                verifying: _self.lang('verifying', 'Verifying'),
              },
              summary: _self.greenIdSummary(),
            },
          );
          ...
          if (typeof GreenId.Overrides.Source[data.sourceId] === 'function') {
            // If there's an override function, update the value
            htmlTemplate = GreenId.Overrides.Source[data.sourceId](htmlTemplate);
          }
      ...
      this.events.subscribe(this.events.const.DISPLAY_CHILD_SOURCE, function(source) {
        ...
        _.each(sources, function(source) {
          ...
          sourceHtml += template(
            '<li class="fc-child-source" data-source="' +
              source.key +
      ...
      this.events.subscribe(this.events.const.SOURCE_SUBMIT, function(source) {
        ...
        if (errors) {
          ...
          _self.events.publish(_self.events.const.VERIFICATION_ERROR, {
            error: _self.lang(
              'requiredFieldsError',
              'Please ensure all required form fields are correctly filled out prior to verification.',
            ),
          });
        } else {
      ...
      this.events.subscribe(this.events.const.VERIFICATION_ERROR, function(obj) {
        ...
        var html = template('<div class="error">' + ' <span><%this.text.error%>' + '</div>', {
          text: {
            error: obj.error,
          },
        });
      ...
      this.events.subscribe(this.events.const.VERIFICATION_SUCCESS, function(obj) {
        var el = _self.getDomElements();
        var statusBar = el.find('.status-bar');
        var html = template('<div class="success">' + ' <span><%this.text.message%>' + '</div>', {
          text: {
            message: obj.msg,
          },
        });
      ...
      this.events.subscribe(this.events.const.VERIFICATION_DO, function(source) {
        ...
        fc.api('greenid/gateway-v2/verify?source=' + source.source, data, 'POST', function(result) {
              ...
              _self.updateRemotely(function(data) {
                ...
                switch (_self.getApplicationStatus()) {
                  case GREENID.STATE.REJECTED:
                    ...
                    _self.events.publish(_self.events.const.FATAL_ERROR, {
                      msg: _self.lang('sorryCouldNotVerify', "We're sorry, but we could not verify your identity."),
                    });
                    return;
                }
                ...
                _self.events.publish(_self.events.const.RENDER_PENDING, {
                  error: _self.lang(
                    'lockedOutTooManyAttempts',
                    "You've been locked out of the selected data source due to too many failed attempts. Please select a different method.",
                  ),
                });
              });
          ...
          if (result.success === true) {
            ...
            if (passedSource(status)) {
              ...
              _self.events.publish(_self.events.const.VERIFICATION_ERROR, {
                error: _self.lang(
                  'verificationFailed',
                  "We're sorry, but electronic identification failed, please try again",
                ),
              });
            }
          ...
          submitGroup.removeClass('verifying');
          _self.events.publish(_self.events.const.VERIFICATION_ERROR, {
            error: _self.lang('unexpectedError', 'An unexpected error occurred when verifying the data source.'),
          });
        });
      });

      // Skipped verification
      this.events.subscribe(this.events.const.VERIFICATION_SKIPPED, function() {
        var el = _self.getDomElements();
        var statusBar = el.find('.status-bar');
        var html = template(
          '<div class="fc-green-id fc-ready">' +
            ...
            '</div>',
          {
            message: _self.lang('skippedVerification', 'You have skipped electronic verification'),
          },
        );
      ...
      // Error when fetching
      this.events.subscribe(this.events.const.FETCH_ERROR, function() {
        // This is deemed a fatal error, the user should be able to pass validation.
        _self.fatalError = true;

        _self.events.publish(_self.events.const.FATAL_ERROR, {
          msg: _self.lang(
            'fetchError',
            'An unknown error occurred initialising electronic verification. You may be asked additional questions later on to verify your identity.',
          ),
        });
      });
      ...
      this.events.subscribe(this.events.const.SOURCE_VERIFIED, function(source) {
        ...
        fc.api('greenid/gateway-v2/fetch', { fieldId: _self.fieldId }, 'POST', function(obj) {
          if (typeof obj.success === 'boolean' && !obj.success) {
            ...
            _self.events.publish(_self.events.const.FETCH_ERROR);
          } else if (typeof obj.data === 'object') {
            if (obj.data.initialised) {
              fc.setVirtualValue(_self.fieldId, obj.data.result, fc.fields, false, false);
            }
            ...
            if (_self.getApplicationStatus() === GREENID.STATE.VERIFIED) {
              ...
              _self.events.publish(_self.events.const.DISPLAY_SUCCESS, {
                msg: _self.lang('identityVerified', 'Your identity has successfully been verified.'),
              });
            } else {
              _self.init(false);
            }
       ...
       * @param set array Array of objects of any type. They are treated as unique.
       * @param k int size of combinations to search for.
       * @returns array
       */
      this.combinations = function(set, k) {
      ...
      this.triggerValidationError = function() {
        _self.events.publish(_self.events.const.RENDER_PENDING, {
          error: _self.lang('mustPassValidation', 'You must pass electronic verification before continuing.'),
        });
      };

      /**
       * Checks all possible combinations of remaining sources to see if any combination can verify the user.
       * If no combination can verify, there is no point in the user proceeding.
       * @returns boolean
       */
      this.canStillVerify = function() {
        // Remaining active sources
        var primarySources = _.keyBy(
          _.filter(_self.options.sources.active, function(o) {
            var status = _self.getSourceStatus(o);
            return (
              o.primary === true &&
              (typeof o.source !== 'string' || _self.userHasSource(o.source)) &&
              status === 'available'
            );
          }),
          'key',
        ...
        // Remaining sources remotely
        var remainingRemoteSources = _.keyBy(
          _.filter(_self.getUserVerificationSources(), function(o) {
            return (
              typeof o.passed === 'boolean' &&
              !o.passed &&
              typeof o.state === 'string' &&
              ['EMPTY', 'IN_PROGRESS'].indexOf(o.state) >= 0
            );
          }),
          'name',
        );

        // Remaining sources locally
        var remainingLocalSources = _.filter(_self.options.sources.active, function(o) {
          var lookupKey = o.key;
          if (typeof o.belongsTo === 'string') {
            lookupKey = o.belongsTo;
          }
          return typeof remainingRemoteSources[o.source] === 'object' && typeof primarySources[lookupKey] === 'object';
        });

        // Retrieve all of the combinations of sources left
        var totalCombinations = [];
        for (var k = 1, l = remainingLocalSources.length; k < l; k++) {
          totalCombinations = _.concat(totalCombinations, _self.combinations(remainingLocalSources, k));
        }

        // Remove combinations where any items exist within the same category (as this is invalid)
        var validCombinations = _.filter(totalCombinations, function(o) {
          var categories = {};
          return _.every(o, function(i) {
            if (typeof i.belongsTo !== 'string') {
              return true;
            }

            if (typeof categories[i.belongsTo] === 'boolean' && categories[i.belongsTo]) {
              return false;
            }

            categories[i.belongsTo] = true;
            return true;
          });
        });

        // Iterate through each valid combination, calculate the sum of each source's matrix, determine if the sum is enough to verify
        var rulesets = _self.getSourceRuleSetWeightings();
        var remainingRequiredMatrix = _self.getRuleSetOutstandingMatrix();

        // Check if the user can still verify
        return _.some(validCombinations, function(c) {
          var canPass = _.every(c, function(source) {
            var ruleset = rulesets[source.source];
            if (typeof ruleset === 'object') {
              _.each(ruleset, function(weight, key) {
                if (typeof ruleset[key] === 'number' && ruleset[key] > 0) {
                  ruleset[key]--;
                }
              });

              // If total sum is 0, verification is still possible
              return _.sum(_.values(ruleset)) === 0;
            }
      ...
      this.getRuleSets = function() {
        return {
          ...
          // Not an active ruleset, but used with K
          Z: {
            total: 2,
            address: 1,
            dob: 2,
       ...
       * @returns {object}
       */
      this.getRuleSetOutstandingMatrix = function(ruleset) {
        ...
        var matrix = this.getMatrix(ruleset);
        ...
        for (var i = 0, l = passed.length; i < l; i++) {
          ...
          for (var key in weightings[source.name]) {
            if (
              weightings[source.name].hasOwnProperty(key) &&
              weightings[source.name][key] > 0 &&
              typeof matrix[key] === 'number' &&
              matrix[key] > 0
            ) {
              --matrix[key];
            }
      ...
      /**
       * Returns the matrix as defined by the config file. When multiple
       * matrices exist (e.g. for ruleset J), it returns the ruleset that yields
       * the highest percentage complete.

       * @param {string} ruleset
       * @returns {object}
       */
      this.getMatrix = function(ruleset) {
        var rulesets = this.getRuleSets();
        var matrices = rulesets[ruleset];

        if (_.isObject(matrices) && !_.isArray(matrices)) {
          // Flat matrix, simply return it
          return matrices;
        }

        if (_.isArray(matrices)) {
          // Ruleset is a combination of multiple rulesets, figure out which one to use
          var max = {
            percentage: -1,
            ruleset: '',
          };

          // Iterate through all matrices, and use the one that yields the highest percentage complete
          _.forEach(matrices, function(ruleset) {
            if (typeof rulesets[ruleset] === 'undefined') {
              // Ruleset isn't defined, return
              return;
            }
            var matrix = rulesets[ruleset];
            var totalItems = _.sum(_.values(matrix));
            var completedItems = totalItems - _.sum(_.values(_self.getRuleSetOutstandingMatrix(ruleset)));
            var percentage = Math.floor(completedItems / totalItems * 10) * 10;

            if (percentage > max.percentage) {
              // If above the maximum, use it
              max = {
          ...
          return this.getMatrix(max.ruleset);
        }
      ...
      this.greenIdSummary = function() {
        ...
        var rulesets = this.getRuleSets();
        var matrix = this.getMatrix(this.options.ruleset);
        var totalItems = _.sum(_.values(matrix));

        // Retrieve the outstanding items
        var outstandingItems = this.getRuleSetOutstandingMatrix();
        var flat = _.values(outstandingItems);
        var outstanding = _.sum(flat);
        var completedItems = totalItems - outstanding;

        // Find the percentage to the nearest 10%
        var percentage = Math.floor(completedItems / totalItems * 10) * 10;
        if (percentage === 0) {
          // No sources completed, don't return a progress bar
          return '';
        }
        ...
        var html = template(
          '<div class="summary">' +
            ...
            '   <div class="bar" style="width: <%this.percentage%>%"><%this.percentage%>% <%this.completeText%></div>' +
            ' </div>' +
            ' <%this.debug%>' +
            '</div>',
          {
            completeText: _self.lang('completeText', 'complete'),
            debug: false ? JSON.stringify(outstandingItems) : '',
            percentage: percentage,
          },
        );
      ...
      /**
       * Checks if the user has skipped
       * @returns bool
       */
      this.isSkipped = function() {
        var value = fc.getValue(_self.fieldId);

        return typeof value.skipped !== 'undefined' && [true, 'true'].indexOf(value.skipped) >= 0;
      };
        ...
        if (source.source === false) {
          ...
          var verifiedChild = _.find(_self.options.sources.active, function(o) {
            return (
              ...
              o.belongsTo === source.key &&
              _self.userHasSource(o.source) &&
              passedSource(sources[o.source])
            );
          });
      ...
      this.hasExtraData = function() {
        return (
          ...
          _.isObject(this.options.sources) &&
          _.isArray(this.options.sources.extraData) &&
          this.options.sources.extraData.length > 0
        );
      };
       ...
       * @returns string
       */
      this.lang = function(id, message) {
        if (typeof GreenId.Overrides.Lang[id] === 'function') {
          // If an override function was passed through, return the result of that function
          return GreenId.Overrides.Lang[id]();
        }

        if (typeof GreenId.Overrides.Lang[id] === 'string') {
          // If a string was passed through, return the string
          return GreenId.Overrides.Lang[id];
        }
      ...
      this.begin = function() {
        return _self.loading(_self.lang('startingUp', 'Starting up identity verification'));
      };

      // Configure the event listeners
      this.configureEventListeners = function() {
        // Configure the event listeners
        var el = fc.domContainer;
        ...
        el.on('click', '.fc-green-id.extra-data-source .source-submit .links a', function() {
          ...
          // Fire the init event
          _self.render(_self.loading(_self.lang('initialising', 'Initialising...')));
          _self.events.publish(_self.events.const.FETCH_RESULT_NOT_INITIALISED, obj.data);

          return false;
        });

        // Verification option skipped
        el.on('click', '.fc-link.verification-skip', function() {
          var value = fc.getValue(_self.fieldId);
          if (typeof value === 'object') {
            value.skipped = true;
          }
          ...
          // Update the value
          fc.setVirtualValue(_self.fieldId, value, fc.fields, false, false);

          // Output the skipped screen
          _self.events.publish(_self.events.const.VERIFICATION_SKIPPED);
        ...
        if (performLookup) {
          ...
          fc.api('greenid/gateway-v2/fetch', { fieldId: this.fieldId }, 'POST', function(obj) {
            if (typeof obj.success === 'boolean' && !obj.success) {
              // There was an error initialising
              _self.events.publish(_self.events.const.FETCH_ERROR);
            } else if (typeof obj.data === 'object' && typeof obj.data.initialised === 'boolean') {
              if (obj.data.initialised) {
