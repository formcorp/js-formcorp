  ...
  return {
    ...
    session: function (fieldId) {
        ...
        fc.api('greenid/gateway-v2/verify?source=' + source.source, data, 'POST', function (result) {
          ...
          if (result.success === true) {
            ...
            if (passedSource(status)) {
              ...
              submitGroup.removeClass('verifying');
              _self.events.publish(_self.events.const.VERIFICATION_ERROR, {
                error: 'Unable to verify data source.'
              });
            }
      ...
      this.events.subscribe(this.events.const.SHOW_EXTRA_DATA_SOURCE, function (obj) {
        console.log('show extra data source');
        console.log(obj);
        var html = template(
          '<div class="fc-green-id">' +
          ' <div class="options">' +
          ...
          '   <div class="source-submit">' +
          '     <div class="links">' +
          '       <a href="#" data-method="agree" class="btn"><%this.agreeText%></a>' +
          '       <a href="#" data-method="disagree" class="btn"><%this.disagreeText%></a>' +
          '     </div>' +
          '   </div>' +
          ' </div>' +
          '</div>'
        , {
          ...
          agreeText: fc.tokenise(obj.preAgree.agreeText || 'Agree'),
          disagreeText: fc.tokenise(obj.preAgree.disagreeText || 'Disagree')
        });
