...
var fcGreenID = (function ($) {
  ...
  var configKeys = {
    ...
    getSourceState = function (field, verificationType) {
      var fieldObj = typeof field === 'string' ? fc.fields[field] : field;
    ...
    updateIsVerified = function (fieldId) {
      ...
      if (fcGreenID.hasSkipped(fieldId)) {
        fieldContainer.removeClass(notVerifiedClass).removeClass(verificationErrorClass).removeClass(verifiedClass).addClass(skippedClass);
      } else if (fcGreenID.isVerified(fieldId)) {
        // Verification has succeeded.
        fieldContainer.removeClass(notVerifiedClass).removeClass(verificationErrorClass).removeClass(skippedClass).addClass(verifiedClass);
      } else if (fcGreenID.hasFailed(fieldId) || fcGreenID.amountOfAvailableSources(fieldId) === 0) {
        // Verification has failed.
        fieldContainer.removeClass(notVerifiedClass).removeClass(verifiedClass).removeClass(skippedClass).addClass(verificationErrorClass);
      } else {
        // Verification has not completed.
        fieldContainer.removeClass(verifiedClass).removeClass(verificationErrorClass).removeClass(skippedClass).addClass(notVerifiedClass);
      }
    },
      ...
      if (parts.length >= 2) {
        ...
        if (iteratorSchema !== undefined) {
          ...
          if (referenceId.length > 0 && fc.fields[referenceId] !== undefined && typeof fc.fields[referenceId][parts[1]] === 'object') {
            $.extend(values, fc.fields[referenceId][parts[1]]);
          }
        }
      }
        ...
        if (dataId !== undefined && typeof dataId === 'string') {
          ...
          if (container.length > 0) {
            ...
            if (typeof value === 'object' && typeof value.result === 'object') {
              fc.fields[dataId].result.outcome = 'SKIPPED';
  ...
  formcorp.customValidators.greenID = function (value, fieldId) {
    return passesValidation(fieldId);
  };

  console.log('init green id validators');
  fc.logic.validators.greenIdVerification = [];
  fc.logic.validators.greenIdVerification.push(formcorp.customValidators.greenID);
