...
(function() {
  ...
  formcorp.modules.IDMatrix.Session = function (fieldId, form, renderer) {
      ...
      _self.form.api(
        ...
        function (data) {
          ...
          if (!data.success) {
            ...
            // Update the form state to error
    ...
    _self.events.subscribe(_self.events.const.DO_ACTIVE_VERIFICATION, function () {
      ...
      activeSources.each(function () {
        ...
        obj.find('select,input[type="text"]').each(function () {
          ...
          if (isRequired && (typeof val !== 'string' || val.length === 0)) {
            ...
            // Check for a custom error
            var validator = input.attr('data-validator');
            if (typeof validator === 'string' && typeof validators[validator] === 'function') {
              var valid = validators[validator](val);
              if (typeof valid === 'object' && typeof valid.error === 'boolean' && valid.error) {
                errors.push(valid.msg);
                _self.showError(valid.msg);
                obj.addClass('error');
                hasError = true;
              }
            }
        ...
        if (sourceErrors > 0) {
          // If errors of the source were detected, delete it
      ...
      // At least one set of values was collected, remove errors
      activeSources.find('select,input[type="text"]').removeClass('fc-error');
